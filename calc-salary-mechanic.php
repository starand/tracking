<?
    ...
    if (isset($_POST['did']) && isset($_POST['amount']) && isset($_POST['formula'])) {
        $did = (int)$_POST['did'];
        get_mechanic($did) or show_error("Такий водій не існує!");
    ...
    $did = (int)$_GET['did'];
    $mechanic = get_mechanic($did) or show_error("Водія не знайдено! '{$_GET['did']}'");
    $routes = get_routes_by_mechanic($did);
?>
<center>
<h2> <?=$mechanic['m_name'];?> </h2>
...
?>
    ...
    <tr>
        <td> &nbsp; <b>Стаж</b>: &nbsp;</td>
        <td> &nbsp; <?=$mechanic['m_stag'];?> &nbsp; <input type='text' id='stag' class='calc-sal' style='width:40px;' disabled value='<?=$mcount;?>'> = <?=$samount;?></td>
