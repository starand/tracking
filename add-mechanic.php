<?
    ...
    checkAuthorizedUser();
    require_permission(ADD.MECHANIC);
    ...
    if (isset($_POST['name']) && isset($_POST['phone']) && isset($_POST['address']) 
        ...
        if (add_mechanic($name, $address, $phone, $idcode, $passport, "0",
            $birthday, $wbirthday, $children, $insurance, $education)) {
            show_message("Автослюсар доданий!");
            load('mechanic.php?did='.last_insert_id(), 'main_space');
        } else {
            show_error("Помилка бази даних. Перевірте чи такий автослюсар вже існує!");
        }
        die();
    }
...
?>
<center>
<form action='add-mechanic.php' method='post' target='submit_frame'>
<table cellspacing='5' cellpadding='1' class='form-table'>
    <TR><TD colspan='2'><center><h2>Додати автослюсаря</h2></TD></TR>
    ...
    <tr><td>Освіта: &nbsp;</td><td><input type='text' name='education' style='width:250px;'></td></tr>
